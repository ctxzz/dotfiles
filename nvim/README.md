...
│       ├── treesitter.lua  # 構文ハイライト（polyglot 置き換え）
...
移行確認が済むまで旧 Vim 設定（`../.vimrc`, `../.vim/`）は**残してあります**。
Neovim で問題なく使えることを確認できたら、以下を削除して構いません：

- `../.vimrc`
- `../.vim/`（`init.vim`, `settings/`, `plugins/`, `autoload/plug.vim` など一式）
- ルート `../.gitignore` 内の `.vim/*` 関連エントリ

> 注意: Vim も併用し続ける場合は削除しないでください。Neovim と Vim は
> 設定ディレクトリが独立しているため、両者は共存できます。
